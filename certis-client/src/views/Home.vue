    ...
    <v-card-text>
      ...
      >
        ...
        <template v-slot:item.person="{ item }">
          <v-tooltip dark color="info" right>
            <template v-slot:activator="{ on, attrs }">
              <span @click="cop(item.person)" v-bind="attrs" v-on="on">{{
                item.person.slice(0, 5) + "..." + item.person.slice(-5)
              }}</span>
            </template>
        ...
        <template v-slot:item.author="{ item }">
          <v-tooltip dark color="info" left>
            <template v-slot:activator="{ on, attrs }">
              <span v-bind="attrs" v-on="on">
                {{item.author}}
              </span>
            </template>
            <span>Chi tiết cơ sở {{item.author}}</span>
          </v-tooltip>
        </template>
